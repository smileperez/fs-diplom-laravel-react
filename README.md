...
| 1.6 | В корневой папке проекта файл `.env.example` переименовываем в `.env. Если файла в папке нет, то необходимо скачать дефолтный файл из инрнета. | Проверяем файл в корневой папке проекта `.env` на корректность, например что адрес, логин, пароль БД стоит корректны (или раскомментируем ци корректно ставим данные БД). | |
...
| 1.8 | Выполняем первую миграцию БД | `php artisan migrate:fresh --seed` |
